...
** replace strings
   ...
   #+begin_src sh
   sed -i 's/v0.8.0-rc9+1/v0.8.0-rc9+1/g' ./stakepool-journey.org
   sed -i 's/nightly/nightly/g' ./stakepool-journey.org
   #+end_src
...
** Avoid evaluating all source code blocks on export
   ...
   #+PROPERTY: header-args :eval never-export
   #+end_example
   on top of this file and pres =C-c C-C= in order to evaluete it.
...
** Generate the doitlive scripts
   ...
   #+begin_src sh
   total 32
   -rw-r--r-- 1 agarciafdz agarciafdz 1400 dic  8 14:47 win01ch2.sh
   -rw-r--r-- 1 agarciafdz agarciafdz  120 dic  8 14:47 win01ch4.sh
   -rw-r--r-- 1 agarciafdz agarciafdz  185 dic  8 14:47 win02ch02.sh
   -rw-r--r-- 1 agarciafdz agarciafdz  871 dic  8 14:47 win02ch03.sh
   -rw-r--r-- 1 agarciafdz agarciafdz  843 dic  8 14:47 win02ch04.sh
   -rw-r--r-- 1 agarciafdz agarciafdz 1099 dic  8 14:47 win02ch05.sh
   -rw-r--r-- 1 agarciafdz agarciafdz  526 dic  8 14:47 win02ch06.sh
   -rw-r--r-- 1 agarciafdz agarciafdz  373 dic  8 14:47 win02ch07.sh
   #+end_src
...
** Generate a compact tutial, just commands

   #+begin_src sh
   cat ./doitlive/win* > /tmp/compact.sh
   sed -i '/^open/d' /tmp/compact.sh
   sed -i '/^clear/d' /tmp/compact.sh
   sed -i '/^#doitlive/d' /tmp/compact.sh
   #+end_src
...
** 2.1 Find the latest version of the testnet
   You can find the configuration paratemers [[https://hydra.iohk.io/job/Cardano/iohk-nix/jormungandr-deployment/latest-finished/download/1/index.html][here]]
...
#+begin_src sh :exports results :tangle ./doitlive/win01ch2.sh :results output code :session jor-server
   cat ./templates/stakepool_config_addenda.json
#+end_src

#+RESULTS[7e93c4cfd0e9a6f320a1d5e134fea6e9be1d2419]:
#+begin_src sh
{
    "log": [
        {
...
that are created in the network/blockchain in real-time. You can check
...
1. execute the =node stats= command from above multiple times and check
   ...
   blockchain values, there should bea new block created every 2-5
   minutes);
...
This step is going to change with the new Daedalus
...
Fund your stake pool account from your /Daedalus Incentivized Wallet/ or
your /Yoroi Incentivized Wallet/ with a regular transaction.

Fund your account with the nightly faucet.

#+name: faucet_results
#+begin_src sh  :tangle ./doitlive/win02ch03.sh :exports both :results output :session jcli-client :dir /ssh:agarciafdz@165.227.57.5:/home/agarciafdz/stakepool
...
check this [[https://input-output-hk.github.io/jormungandr/stake_pool/registering_stake_pool.html][article]]. Remember the cuantities are in *Lovelaces* i.e. 1 ADA = 1,000,000 Lovelace.
...
** 4.2 Restart Jormungandr as a stake pool
   ...
   In order to do that return to the same terminal where you starte jormungandr and stop the sever.
...
** 5.3 Sign with your owner Private Key
   We need to make sure that the person maing the submission is the owner of the Public key in the json file.
...
** 7.1 Check delegated stake through JCLI
   ...
   #+begin_src sh  :tangle ./doitlive/win02ch07.sh :exports botht :results value org :session jcli-client :dir /ssh:agarciafdz@165.227.57.5:/home/agarciafdz/stakepool
